...
class ArtistWithMoreThanOneIllusPerCard extends AbstractCardsReport
{

    public function GetReportName(): string { return('ArtistWithMoreThanOneIllusPerCard'); }
...
SELECT 
	card.oracle_id,
    name,
	COUNT(DISTINCT(`artist`)) AS distinct_artist_count, 
    COUNT(DISTINCT(`name`)) AS distinct_cards_count,
    COUNT(DISTINCT(`illustration_id`)) AS distinct_ill_count
...
JOIN lore.card ON 
    ...
    cardface.card_id
GROUP BY name
HAVING distinct_artist_count < distinct_ill_count
